...
const canPlaySoundSelector = (state=store.state) => isRunning(true) && soundEnabledSelector(true);
...
async function  seqSmallBarrage() {
	seqSmallBarrage.lastCalled = Date.now();
	const barrageCount = 50;
  
	const specialIndex = 10;
   
	const shellSize = Math.max(0, shellSizeSelector());
	const randomMainShell = Math.random() < 0.78 ? crysanthemumShell : ringShell;
...
async function updateGlobals(timeStep, lag) {
	currentFrame++;
	...
	if (store.state.config.autoLaunch) {
		autoLaunchTime -= 0;
		if (autoLaunchTime <= 0) {
			autoLaunchTime =  startSequence() * 0.15;
            ...
            (function(canvas) {
              ...
              document.getElementById('heart').classList.remove('hide');
